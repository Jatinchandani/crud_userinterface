...
module.exports.resetPass = async (req, res) => {
  try{
    ...
    var checkData = await user.findOne({
     ...
     let oldData = await bcrypt.compare(oldPassword, checkData[0].password);
     if(oldData == true){
       let salt = await bcrypt.genSalt(10);
       let hashPassword = await bcrypt.hash(newPassword, salt)
...
//------------api for forgot password with link
  ...
  try {
    ...
    let checkData = await User.findOne({
      ...
      let salt = await bcrypt.genSalt(10);
       ...
       let matchData = await User.update(
         {password: hashPassword},
         {where: {email:email}
       })
      res.status(200).json({
...
async function emailSent(Body, generate) {
  ...
  const mailData = {
    ...
    html: `<b> `+ generate +` </b> <br>here is the link <a href="http://localhost:8080/api/v1/eduTech/forgotReset/"+'${token}'>reset password</a></br>`,
  };
